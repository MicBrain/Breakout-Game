...
(define (merge comp list1 list2)
    (cond ((null? list2) list1) 
          ...
          (else (cons (car list1) (merge comp list2 (cdr list1))))))
...
(define (partition total max-value max-pieces values)
  (cond ((= total 0) (list values))
    ((< total 0) nil)
    ((= max-value 0) nil)
    ((= max-pieces 0) nil)
    (else (append (partition total (- max-value 1)  max-pieces values) 
    (partition (- total max-value) max-value (- max-pieces 1) (cons max-value values))))))
(partition total max-value max-pieces '()))
...
(define (tree-sums tree)
  (define (element_list make_deeper) 
    (if (null? make_deeper) nil (append (car make_deeper) (element_list (cdr make_deeper)))))
  (define (all_adder adder value) (define (item another) (+ adder another)) (map item value))
    (if (null? (children tree)) (cons (entry tree) nil)
    (all_adder (entry tree) (element_list (map tree-sums (children tree))))))
