...
public class Homework3 {
  ...
  public static void smoosh(int[] ints) {
    ...
    int checker = ints[0];
    int reassignNumIndex = 0;
    int tracker;
    int indexOfMinusOne;
    int length = ints.length;
    for (tracker = 1; tracker<ints.length; tracker++) {
      if (ints[tracker] != checker) {
        reassignNumIndex++;
        checker = ints[tracker];
        ints[reassignNumIndex] = ints[tracker];         
      }else {
        checker = ints[tracker];
      }      
    }
    for (indexOfMinusOne=reassignNumIndex+1; indexOfMinusOne<length; indexOfMinusOne++) {
      ints[indexOfMinusOne] = -1;
    }
  }
  ...
  public static void main(String[] args) { 
    ...
    int[] test9 = {4, -1, -1, 4, -1, 0, -1, -1};
    System.out.println("smooshing " + stringInts(test9) + ":");
    smoosh(test9);
    result = stringInts(test9);
    System.out.println(result);
    TestHelper.verify(result.equals("[  4  -1  4  -1  0  -1  -1  -1  ]"),
                      "BAD SMOOSH!!!  No cookie.");
